...
class DatabaseConnector:
    def __init__(self):
        ...
        # Garante que o código não quebre se os segredos não estiverem lá (para testes locais)
        try:
            ...
            st.sidebar.error("Arquivo de segredos não configurado. Usando dados de simulação.")
        
        self.conn = None
        
    def connect(self):
        # Só tenta conectar se os drivers e os segredos estiverem disponíveis
        if not PYODBC_AVAILABLE or not self.SECRETS_AVAILABLE:
            st.sidebar.warning("Conexão com banco desativada. Usando dados de simulação.", icon="🔌")
            return False
    ...
    def get_data(self):
        # --- Query principal alinhada com o esquema de BD fornecido ---
        ...
        try:
            ...
            if df.empty:
                st.error("A query ao banco foi bem-sucedida, mas não retornou dados.")
                return pd.DataFrame()
            return df
        except Exception as e:
...
class VoiceCommandProcessor:
    def process(self, command):
        ...
        if re.search(r"limpar|resetar filtros", command):
            return "Filtros redefinidos para o estado inicial. (Funcionalidade em desenvolvimento)"
...
with st.sidebar:
    ...
    # Prepara listas de filtros
    # Garante que, mesmo que os dados estejam vazios, o app não quebre
    ...
    # Filtros Dinâmicos
    ano_selecionado = st.selectbox("Ano", ["TODOS"] + anos)
    mes_selecionado = st.selectbox("Mês", ["TODOS"] + meses)
    consultor_selecionado = st.multiselect("Consultores", ["TODOS"] + consultores, default=["TODOS"])
    cliente_selecionado = st.multiselect("Clientes", ["TODOS"] + clientes, default=["TODOS"])
    
    # --- CORREÇÃO DO SYNTAXERROR ESTÁ AQUI ---
    projeto_selecionado = st.multiselect("Projetos", ["TODOS"] + projetos, default=["TODOS"])

    if st.button("Aplicar Filtros", use_container_width=True, type="primary"):
        # A lógica de aplicação de filtro agora está na renderização das abas
        pass
...
# Calcula KPIs GLOBAIS com base nos filtros
if not df_filtrado.empty:
    ...
    # Define KPIs como zero se não houver dados
...
with tabs[0]:
    ...
    if df_filtrado.empty:
        ...
        with c2:
            # --- Gráfico Lucro por Tipo de Projeto (Similar à Tela 3 do Vídeo) ---
...
with tabs[1]:
    ...
    if df_filtrado.empty:
        ...
        # --- Gráfico Risco vs. Recompensa (Tela 3 do Vídeo) ---
...
with tabs[2]:
    # --- Tabela de Fechamento por Consultor (Tela 4 do Vídeo) ---
...
# Tab 5: Fechamento
with tabs[4]:
    # --- Tabelas de Fechamento (Tela 4 do Vídeo) ---
    st.subheader("Painel de Fechamento (Visão Contábil)")
    
    # --- MAPA: Nota sobre a Visão Caixa vs. Contábil ---
    st.info("""
...
# Tab 6: Assistente IA
with tabs[5]:
    # --- Insights Preditivos (Tela 2 do Vídeo) ---
    ...
    if comando_usuario:
        with st.spinner("Processando seu comando..."):
            ...
            if "Navegando" in resposta:
                # Força a atualização da interface para mudar de aba
                st.rerun()
