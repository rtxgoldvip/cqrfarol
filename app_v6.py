...
def run_query(query, _conn):
    ...
    # Adicionar verificação se a conexão é válida
    if not _conn:
...
class CoreQuantumReasoning:
    ...
    def load_universo_dados(self):
        ...
        with st.spinner("Conectando e buscando dados mestre..."):
            for name, query in QUERIES.items():
                ...
                if dfs[name].empty:
                    ...
                    if name == 'g': # Se a tabela FATO falhar, não adianta continuar
                        all_loaded = False


        # 3. Verificar se as tabelas FATO e DIM principais existem
        if not all_loaded or 'g' not in dfs or dfs['g'].empty:
            st.error("Tabela Fato (Tb_GestorFin2) está vazia ou falhou ao carregar. Análise impossível.")
            return pd.DataFrame()
        ...
        with st.spinner("Entrelaçando dimensões (Joins)..."):
            ...
            def safe_merge(df_left, df_right, **kwargs):
                if df_right.empty:
                    ...
                    return df_left
                # Garantir que as chaves de merge existam nos DFs
                left_key = kwargs.get('left_on')
                right_key = kwargs.get('right_on')
                if left_key and left_key not in df_left.columns:
                    st.warning(f"Skipping merge: Chave '{left_key}' não encontrada no DataFrame esquerdo.")
                    return df_left
                if right_key and right_key not in df_right.columns:
                    st.warning(f"Skipping merge: Chave '{right_key}' não encontrada no DataFrame direito.")
                    # Cria a coluna chave vazia no DF direito para evitar erro no merge
        ...
        with st.spinner("Mapeando colunas e criando métricas..."):
            ...
            # Certificar que Ano e Mes são inteiros antes de criar Data
            ...
            df['Status'] = pd.cut(df['Score_Performance'],
                                 bins=[-np.inf, 40, 70, np.inf], # Ajustado para incluir <0 e >100
                                 labels=['CRÍTICO', 'ATENÇÃO', 'EXCELENTE'],
                                 right=False) # Mudado para right=False para incluir 0 em CRÍTICO

            # ----------------- CORREÇÃO DO BUG AQUI (v3) -----------------
            ...
            for col_str in colunas_string_app:
                if col_str not in df.columns:
                    ...
                    # Convertendo para string ANTES de fillna para evitar erros de tipo
    ...
    def aplicar_colapso_quantico(self, filtros):
        ...
        """
        # Adiciona verificação se dados_universo está vazio
        ...
        try:
            ...
            if filtros.get('mes') and filtros.get('ano'):
                ...
                # Certificar que Mes e Ano no df são int antes de comparar
                df = df[(df['Mes'].astype(int) == mes_sel) & (df['Ano'].astype(int) == ano_sel)]

            self.estado_quantum = df
            return df
        except Exception as e:
            st.error(f"Erro ao aplicar filtros: {e}")
            self.estado_quantum = pd.DataFrame() # Retorna DF vazio em caso de erro
            return self.estado_quantum
    ...
    def detectar_entrelacements(self):
        ...
        if len(df) >= 5 and 'Eficiencia' in df.columns and 'Margem' in df.columns:
            try:
                corr_efic_margem = df[['Eficiencia', 'Margem']].corr().iloc[0, 1]
                if pd.notna(corr_efic_margem) and abs(corr_efic_margem) > 0.6:
                ...
                st.warning(f"Erro ao calcular correlação Eficiencia/Margem: {e}")


        # Análise por consultor
        if 'Consultor' in df.columns and 'Score_Performance' in df.columns:
            try:
                perf_consultor = df.groupby('Consultor').agg({
                    'Score_Performance': 'mean',
                    'ROI_Hora': 'mean',
                    'Margem': 'mean'
                }).dropna() # Remover consultores com NaN
                perf_consultor = perf_consultor[perf_consultor.index != 'N/A'] # Remover 'N/A'

                if len(perf_consultor) > 1:
                    variancia = perf_consultor['Score_Performance'].std()
                    if pd.notna(variancia) and variancia > 15:
                 ...
                 st.warning(f"Erro ao analisar performance por consultor: {e}")


        # Análise por tipo de projeto
        if 'TipoProj' in df.columns and 'ROI_Hora' in df.columns:
            try:
                perf_tipo = df.groupby('TipoProj').agg({
                    'Margem': 'mean',
                    'ROI_Hora': 'mean'
                }).dropna() # Remover tipos com NaN
                perf_tipo = perf_tipo[perf_tipo.index != 'N/A'] # Remover 'N/A'
                ...
                st.warning(f"Erro ao analisar performance por tipo de projeto: {e}")
    ...
    def gerar_prescricoes_quantum(self):
        ...
        if 'otimizacao_mix' in entrelacements:
            ...
            if info['melhor'] in perf_tipo.index and info['pior'] in perf_tipo.index:
                ...
                st.warning("Skipping prescrição 'otimizacao_mix': Tipos de projeto não encontrados após filtragem/agregação.")
        ...
        if len(prescricoes) == 0 or (len(prescricoes) == 1 and prescricoes[0]['prioridade'] == 'CRÍTICA' and "FLUXO DE CAIXA 2025" in prescricoes[0]['titulo']):
             # Verifica se a ÚNICA prescrição é a do GAP de dados
...
class SocraticQuestioningEngine:
    ...
    def _questionar_rentabilidade(self, df):
        ...
        margem_media = df['Margem'].mean()
        if pd.notna(margem_media) and margem_media < 0.35: # Ajustado para fração 0.xx
            perguntas.append({
    ...
    def _questionar_talento(self, df):
        perguntas = []
        if 'Consultor' in df.columns and len(df['Consultor'].unique()) > 2:
    ...
    def _questionar_portfolio(self, df):
        perguntas = []
        if 'Cliente' in df.columns and len(df['Cliente'].unique()) > 1:
    ...
    def _questionar_crescimento(self, df):
        perguntas = []
        if 'Data' in df.columns and not df['Data'].isnull().all() and len(df['Data'].unique()) > 1:
            df_temp = df.dropna(subset=['Data'])
            receita_tempo = df_temp.groupby('Data')['Receita'].sum().sort_index()

            if len(receita_tempo) >= 2:
                # Calcular crescimento médio, ignorando NaNs
                crescimento_vals = receita_tempo.pct_change().dropna()
                crescimento = crescimento_vals.mean() * 100 if not crescimento_vals.empty else 0

                if pd.notna(crescimento) and crescimento < -5:
                    perguntas.append({
                        'categoria': 'CRÍTICO',
    ...
    def _questionar_riscos(self, df):
        perguntas = []
        if 'Margem' in df.columns and 'Lucro' in df.columns:
    ...
    def _questionar_pricing(self, df):
        ...
        if 'VH_Venda' in df.columns and 'VH_Custo' in df.columns:
            ...
            df_temp = df[(df['VH_Custo'] > 0) & pd.notna(df['VH_Venda']) & pd.notna(df['VH_Custo'])]
            if not df_temp.empty:
                df_temp['Markup'] = (df_temp['VH_Venda'] - df_temp['VH_Custo']) / df_temp['VH_Custo'] * 100
                markup_medio = df_temp['Markup'].mean()

                if pd.notna(markup_medio) and markup_medio < 80:
                    perguntas.append({
                        'categoria': 'FINANCEIRO',
...
if 'socratic_engine' not in st.session_state:
    # Apenas inicializa se o CRQ foi carregado com sucesso
    if st.session_state.crq and not st.session_state.crq.dados_universo.empty:
        st.session_state.socratic_engine = SocraticQuestioningEngine(st.session_state.crq)
         ...
         st.session_state.socratic_engine = None # Marca como não inicializado
...
with st.sidebar:
    ...
    # Extrair opções únicas do DF carregado (com tratamento de erro)
    try:
        ...
        st.error("Verifique se as colunas Consultor, Cliente, Projeto, TipoProj, Mes, Ano existem após o merge.")
        # Define listas vazias para evitar mais erros
    ...
    # Gerar prescrições apenas se Socratic Engine foi inicializado
    prescricoes = crq.gerar_prescricoes_quantum() if ia_ativa and socratic else []
    ...
    if st.button("🔄 Reprocessar Dados", use_container_width=True):
        ...
        # Limpar o estado para forçar a recriação do CRQ
...
with tab1:
    ...
    with col3:
        margem_display = metricas['margem'] * 100 if pd.notna(metricas['margem']) else 0
        st.metric(
            "📈 Margem Média",
            f"{margem_display:.1f}%", # Convertendo fração para %
    ...
    with col_viz1:
        ...
        if not df_filtrado.empty:
            try:
                ...
                rec_cliente = rec_cliente[rec_cliente.index != 'N/A'] # Remover N/A
    ...
    with col_viz2:
        ...
        if not df_filtrado.empty:
            try:
                ...
                perf_cons = perf_cons[perf_cons.index != 'N/A'] # Remover N/A
                ...
                st.warning(f"Erro ao gerar gráfico Performance por Consultor: {e}")
        else:
            st.info("🎯 Sem dados disponíveis para este filtro")
    ...
    with col_pag:
        ...
        if not df_filtrado.empty:
            try:
                ...
                apagar = apagar[apagar['Consultor'] != 'N/A'] # Remover N/A
                ...
                total_custo_contabil = apagar['Total_Custo_Contabil'].sum()
                total_pago = apagar['Total_Pago'].sum()
                saldo_pendente_total = apagar['Saldo_Pendente'].sum()

                st.metric("Total Custo Contábil", f"R$ {total_custo_contabil:,.2f}")
                st.metric("Total Efetivamente Pago", f"R$ {total_pago:,.2f}",
                          delta=f"R$ {saldo_pendente_total:,.2f} Pendente",
                          delta_color="inverse" if saldo_pendente_total > 0 else "off")
            except Exception as e:
    ...
    with col_rec:
        ...
        if not df_filtrado.empty:
            try:
                ...
                areceber = areceber[areceber['Cliente'] != 'N/A'] # Remover N/A
                ...
                total_faturado = areceber['Total_Faturado'].sum()
                total_recebido = areceber['Total_Recebido'].sum()
                saldo_pendente_receber = areceber['Saldo_Pendente'].sum()

                st.metric("Total Faturado (Contábil)", f"R$ {total_faturado:,.2f}")
                st.metric("Total Efetivamente Recebido", f"R$ {total_recebido:,.2f}",
                          delta=f"R$ {saldo_pendente_receber:,.2f} Pendente",
                          delta_color="inverse" if saldo_pendente_receber > 0 else "off")
            except Exception as e:
...
with tab3:
    ...
    if ano_sel >= 2025 and metricas['caixa_recebido'] == 0 and metricas['caixa_pago'] == 0 and metricas['receita'] > 0:
    ...
    with st.spinner("Calculando evolução temporal..."):
        ...
        if not df_full.empty and 'Data' in df_full.columns and not df_full['Data'].isnull().all():
            df_full_temp = df_full.dropna(subset=['Data']).copy() # Trabalhar com cópia

            try:
                fluxo_temporal = df_full_temp.groupby(pd.Grouper(key='Data', freq='MS')).agg(
...
with tab4:
    ...
    if not df_filtrado.empty:
        try:
            ...
            tipo_fechado = "PROJETO FECHADO" # Valor padrão da sua spec
            tipos_presentes = df_filtrado['TipoProj'].unique()

            if tipo_fechado not in tipos_presentes:
                 # Tenta achar um nome similar se o nome exato falhar
                 matches = [t for t in tipos_presentes if isinstance(t, str) and "FECHADO" in t.upper()]
                 if matches:
            ...
            if tipo_fechado:
                ...
                st.warning("Não foi possível executar a análise de sangria (tipo de projeto não encontrado ou N/A).")
        except Exception as e:
    ...
    if not df_filtrado.empty and len(df_filtrado) > 3:
        try:
            ...
            # Selecionar apenas colunas que realmente existem no df_filtrado
...
with tab6:
    ...
    if not df_filtrado.empty and socratic: # Verifica se socratic foi inicializado
        ...
        if perguntas:
            categorias_disponiveis = sorted(list(set([p['categoria'] for p in perguntas])))
            ...
            if perguntas_filtradas:
                for i, pergunta in enumerate(perguntas_filtradas, 1):
                    ...
                    # ------------- CORREÇÃO DO SYNTAX ERROR AQUI (v4) -------------
                    st.markdown(f"""
                    <div class="{card_class}">
                    ...
                    """, unsafe_allow_html=True) # <<< FECHAMENTO CORRETO AQUI
                    # ----------------------------------------------------------------
